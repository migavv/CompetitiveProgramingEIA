...
public:
    ...
    int negatives(int a, int b) {return negatives(b) - negatives(a - 1); } //Returns frequency of zero from a to b
    ...
    void adjust(int k, int v) {
        for(int i = k; i < ft.size(); i += LSOne(i)) {
            if(!v) {
                if(ft[k].val) ft[i].z++; //If a number != is replaced by a 0, increase zeroes counter
                ...
                if(ft[k].val == 0) ft[i].z--; //If the number  was negatives, decrease zeroes counter
            }
            else {
                if(ft[k].val < 0) ft[i].n--; //If a negative number is replaced by a positive number, decrease negatives counter
                else if(!ft[k].val) ft[i].z--; //If a zero is replaced by a positive number, decrease zeroes counter
            }
        }
        ft[k].val = v;
    }
